...
public class RabbitMQConfig4 {
    ...
    //1.2 创建2个队列
    @Bean
    public Queue createT1(){  //队列1
        return new Queue("q_topic_01");
    }
    @Bean
    public Queue createT2(){ //队列2
    ...
    public Binding createB1(TopicExchange te){
        //把队列绑定到交换器上
        //with("log1")：路由关键字
        return BindingBuilder.bind(createT1()).to(te).with("user.*");//绑定队列1
    }
    @Bean
    public Binding createB2(TopicExchange te){
        return BindingBuilder.bind(createT2()).to(te).with("order.#");//绑定队列2
    }
}
